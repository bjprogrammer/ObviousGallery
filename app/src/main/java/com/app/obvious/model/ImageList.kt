...
class ImageList private constructor(parcel: Parcel) : BaseObservable(), Parcelable {
    @SerializedName("Data")
    private var data: List<Image>? = null

    @Bindable
    fun getData(): List<Image>? = data
    ...
    init {
        parcel.readTypedList(data?.toMutableList()?: emptyList(), Image.CREATOR)
    }
    ...
    companion object {
        ...
        val CREATOR: Parcelable.Creator<ImageList> = object : Parcelable.Creator<ImageList> {
            override fun createFromParcel(parcel: Parcel): ImageList {
                return ImageList(parcel)
            }

            override fun newArray(size: Int): Array<ImageList?> {
                return arrayOfNulls(size)
            }
        }
    }
